...
func NewRedis(ctx context.Context, opts *RedisOpts) *Redis {
	...
	return NewRedisWithConn(ctx, conn)
}

// NewRedisWithConn 实例化
func NewRedisWithConn(ctx context.Context, conn redis.UniversalClient) *Redis {
...
// TTL get remaining time to live of a cached value.
func (r *Redis) TTL(key string) (int, error) {
	ttl := r.conn.TTL(r.ctx, key)
	return int(ttl.Val().Seconds()), ttl.Err()
}
