...
func (s *Server) AuthUrl(isWebAuth bool, redirectUri string, authType AuthType, bizAppid *string) (u string, err error) {
	...
	if isWebAuth {
		u = fmt.Sprintf(WEB_AUTH_URL, s.cfg.AppID, resp.PreAuthCode, redirectUri, authType)
	} else {
		u = fmt.Sprintf(MOBILE_AUTH_URL, s.cfg.AppID, resp.PreAuthCode, redirectUri, authType)
